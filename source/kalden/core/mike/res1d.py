...
* Keep optional spatial helpers for node/reach GeoDataFrames and GeoPackage exports.
...
def _require_spatial_dependencies():
    ...
    try:
        ...
        raise ImportError(
            "Spatial res1d helpers require geopandas and shapely. Install them "
            "with something like: pip install geopandas shapely pyogrio"
        ) from exc
...
def _summary_column_name(reducer: str, quantity: str) -> str:
    """Return a readable default output column for a summary quantity."""

    snake = re.sub(r"(?<!^)(?=[A-Z])", "_", str(quantity)).lower()
...
class Res1D:
    ...
    crs : str, int, or None, default None
        Coordinate reference system associated with geometry in the result file,
        for example ``"EPSG:2056"`` or ``2056``. It is used by optional spatial
        helpers such as :meth:`reaches_gdf` and :meth:`summary_gdf`.
    ...
    def __init__(
        ...
        *,
        ...
        # Backward-compatible cache stems used by the older exploratory helper.
        # Spatial helpers write/read ``nodes.*`` and ``reaches.*`` directly under
        # ``cache_dir`` so old notebooks can still do:
        # ``res._read_spatial_cache(res.cache_dir / "reaches")``.
        self.index_stem = self._cache_stem("series_index")
        self.nodes_stem = self.cache_dir / "nodes"
    ...
    @staticmethod
    def _spatial_candidates(stem: Path) -> list[tuple[str, Path]]:
        """Return spatial cache candidates for a stem path."""

        return [
            ("parquet", stem.with_suffix(".parquet")),
            ("gpkg", stem.with_suffix(".gpkg")),
            ("geojson", stem.with_suffix(".geojson")),
        ]

    def _spatial_cache_stem(self, object_type: str) -> Path:
    ...
    def _read_spatial_cache(self, stem: PathLike) -> Any | None:
        ...
        This method is intentionally kept for backward compatibility with older
        notebooks. Prefer the public methods :meth:`spatial_index`,
        ...
        for fmt, file_path in self._spatial_candidates(stem_path):
            if not file_path.exists():
                continue
            if fmt == "parquet":
                try:
    ...
    def _write_spatial_cache(self, gdf: Any, stem: PathLike) -> Path | None:
        """Write a GeoDataFrame cache, preferring parquet then GPKG/GeoJSON."""
        ...
        raise OSError("Could not write spatial cache file (" + "; ".join(errors) + ")")

    def _node_geometry(self, node: Any) -> Any | None:
        """Return a Shapely point for a node when coordinates can be found."""

        _gpd, _line_string, Point = _require_spatial_dependencies()

        def point_from_xy(obj: Any) -> Any | None:
            if obj is None:
                return None
            ...
            x_float = _as_float_or_none(x)
            y_float = _as_float_or_none(y)
            if x_float is not None and y_float is not None:
                return Point(x_float, y_float)
            return None

        point = point_from_xy(node)
        ...
        for attr in [
            ...
            obj = _safe_getattr(node, [attr])

            point = point_from_xy(obj)
            if point is not None:
                return point

            if isinstance(obj, (list, tuple)) and len(obj) >= 2:
                x_float = _as_float_or_none(obj[0])
                y_float = _as_float_or_none(obj[1])
                if x_float is not None and y_float is not None:
                    return Point(x_float, y_float)

        return None

    def _reach_geometry(self, reach: Any) -> Any | None:
        """Return a Shapely line for a reach when geometry points can be found."""

        _gpd, LineString, _point = _require_spatial_dependencies()
        ...
        for point in points:
            x = _safe_getattr(point, ["x", "X", "c", "XCoordinate"])
            y = _safe_getattr(point, ["y", "Y", "YCoordinate"])
            x_float = _as_float_or_none(x)
            y_float = _as_float_or_none(y)
            if x_float is not None and y_float is not None:
                coordinates.append((x_float, y_float))
    ...
    def build_spatial_index(
        ...
        *,
        ...
        """Build/cache node and reach GeoDataFrames.

        This restores the older spatial-index workflow while keeping GeoPandas
        optional. It also caches separate ``weirs`` and ``pumps`` layers when
        those special reach-backed objects are present.

        Returns
        -------
        tuple
            ``(nodes_gdf, reaches_gdf)`` for compatibility with the older
            ``Res1DExplorer.build_spatial_index`` API.
        """
        ...
        if not force_refresh:
            nodes = self._read_spatial_cache(self.nodes_stem)
            reaches = self._read_spatial_cache(self.reaches_stem)
            if nodes is not None and reaches is not None:
    ...
    def nodes_gdf(self, *, force_refresh: bool = False) -> Any:
        ...
        return self.spatial_index("node", force_refresh=force_refresh)

    def reaches_gdf(self, *, force_refresh: bool = False) -> Any:
        """Return the regular reach spatial index as a GeoDataFrame."""

        return self.spatial_index("reach", force_refresh=force_refresh)
    ...
    def summary_gdf(
        ...
        *,
        ...
        This is the general version of your old notebook loop. It reads each
        selected time series one by one through :meth:`read_series`, so it uses
        the normal lazy cache and does not load all series into memory at once.

        Parameters
        ----------
        quantity : str
            Result quantity to aggregate, for example ``"ReachFilling"``.
        object_type : str, default ``"node"``
            Object type to summarize: ``node``, ``reach``, ``weir``, or ``pump``.
        reducer : {"max", "min", "mean", "last"}, default ``"max"``
            Aggregation across all numeric columns and all timesteps.
        output_column : str, optional
            Name of the output attribute. Defaults to e.g. ``max_reach_filling``.
        errors : {"raise", "warn", "ignore"}, default ``"warn"``
            Error handling when one object cannot be read.
        show_progress : bool, default False
            Use ``tqdm`` when available.
        """

        reducer_normalized = reducer.strip().lower()
        if reducer_normalized not in {"max", "min", "mean", "last"}:
            raise ValueError("reducer must be one of: max, min, mean, last")
        if errors not in {"raise", "warn", "ignore"}:
            raise ValueError("errors must be one of: raise, warn, ignore")

        normalized_type = _normalize_object_type(object_type)
        value_column = output_column or _summary_column_name(
            reducer_normalized,
            quantity,
        )
        ...
        if show_progress:
            try:
                ...
                iterator = tqdm(object_ids, desc=f"{reducer_normalized} {quantity}")
            except Exception:
                iterator = object_ids
        ...
        for object_id in iterator:
            try:
                ...
                if numeric.empty:
                    values[str(object_id)] = None
                    continue

                if reducer_normalized == "max":
                    value = numeric.max().max()
                elif reducer_normalized == "min":
                    value = numeric.min().min()
                elif reducer_normalized == "mean":
                    value = numeric.mean().mean()
                else:  # reducer_normalized == "last"
                    value = numeric.iloc[-1].mean()

                values[str(object_id)] = float(value) if pd.notna(value) else None
            except Exception as exc:
                values[str(object_id)] = None
                message = (
                    f"Could not read {quantity} for "
                    f"{normalized_type}/{object_id}: {exc}"
                )
                if errors == "raise":
                    raise RuntimeError(message) from exc
                if errors == "warn":
    ...
    def export_summary_gpkg(
        self,
        output_path: PathLike,
        *,
        quantity: str,
        object_type: str = "reach",
        reducer: str = "max",
        output_column: str | None = None,
        ...
        """Export an aggregated spatial summary to a GeoPackage layer."""

        gdf = self.summary_gdf(
            quantity=quantity,
            object_type=object_type,
            reducer=reducer,
            output_column=output_column,
        ...
        layer = layer_name or f"{reducer}_{object_type}_{quantity}"
        gdf.to_file(output, layer=layer, driver="GPKG")
        return output

    def export_max_reach_filling_gpkg(
        self,
        output_path: PathLike,
        *,
        ...
        """Export maximum reach filling to a GeoPackage layer.

        The exported attribute defaults to ``max_filling`` so your existing QML
        style can keep referencing that field name.
        """
    ...
    def timeseries_frame(
        ...
        *,
        ...
        """Return one result series in long format for plotting.

        This is a thin convenience wrapper around :meth:`read_series`. Plotting
        helpers are imported lazily so importing :mod:`res1d` does not require
        optional notebook/plotting dependencies.
        """

        from .res1d_widgets import normalize_timeseries_for_plot
    ...
    def plot_series(
        ...
        *,
        ...
        """Plot one lazily-read and cached result series.

        ``backend`` can be ``"plotly"`` or ``"matplotlib"``. The corresponding
        plotting package is imported only when this method is called.
        """

        from .res1d_widgets import plot_timeseries
    ...
    def timeseries_widget(
        ...
        """Return an interactive notebook widget for browsing time series.

        The widget implementation lives in :mod:`res1d_widgets` to keep this
        reader module dependency-light. Install optional dependencies such as
        ``ipywidgets`` and ``plotly`` only where the widget is needed.

        Parameters
        ----------
        keep_history : bool, default False
            If True, changing the selected object/quantity appends plots to the
            output area instead of replacing the previous plot. This keeps only
            the rendered widget output/history in the notebook, not every time
            series DataFrame in memory.
        """

        from .res1d_widgets import res1d_timeseries_widget

        return res1d_timeseries_widget(
            explorer=self,
